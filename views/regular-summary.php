<div class="w-full relative">
    ...
    <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-3xl font-bold text-slate-900 tracking-tight">Regular Summary</h1>
            <p class="text-slate-500 mt-1 font-medium text-sm">Detailed inventory tracking of all regular fund cluster land asset properties</p>
        </div>
...
<script>
    ...
    async function loadAssets() {
        const response = await fetch('/dar/api/assets.php?cluster=regular');
            ...
            <tr id="row-${id}" class="hover:bg-slate-50/50 transition-all duration-150 cursor-pointer border-l-4 border-l-transparent" onclick="toggleDetails(${id})">
                ...
                <td class="py-5 px-6 text-center" onclick="event.stopPropagation();">
                    <div class="flex items-center justify-center gap-2">
                        <a href="/dar/regular-summary/edit?id=${id}" class="p-2 bg-slate-100 hover:bg-emerald-50 hover:text-emerald-700 text-slate-600 rounded-xl transition-all duration-200">
